...
function findParentPath(path1, path2) {
    ...
    _.forEach(path1, function(c, idx) {
    ...
    if (
        !(
            commonPath.length === path1.length ||
            ...
            function vasyncDone(err, results) {
                ...
                if (recursive) {
                    ...
                    if (currentPath === '/') {
                        return callback(
                            new Error(
                                'Traversed up to root directory, but ' +
                                    'could not find ' +
                                    itemName +
                                    '!'
                            )
    ...
    vasync.waterfall(
        [
            function findNmPath(innerCb) {
                findItem(NODE_MODULES, 'directory', currentPath, true, function(
                    findErr,
                    nmPath
                ) {
                    if (findErr) {
                        return innerCb(
                            new WError(
                                findErr,
                                'Could not find node_module ' + moduleName
                            )
                        );
                    }
        ...
        vasync.forEachParallel(
            ...
            function(_ignore, results) {
                ...
                if (successOp) {
                    ...
                    return callback(
                        new Error(
                            'failed to find any variation of ' +
                                currentPath +
                                ' + ' +
                                item
                        )
